...
export const automationTriggerQuery = extendType({
  ...
  definition(t) {
    ...
    t.field('findFirstAutomationTrigger', {
      type: 'AutomationTrigger',
      args: {
        where: 'AutomationTriggerWhereInput',
        orderBy: arg({ type: 'AutomationTriggerOrderByInput' }),
        cursor: 'AutomationTriggerWhereUniqueInput',
        skip: 'Int',
        take: 'Int',
      },
      async resolve(_root, args, ctx) {
        return ctx.prisma.automationTrigger.findFirst(args as any)
      },
    })
    ...
    t.field('automationTriggersCount', {
      ...
      async resolve(_root, args, ctx) {
        return ctx.prisma.automationTrigger.count(args as any)
      },
    })
  },
})
