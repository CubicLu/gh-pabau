...
export const bookingQuery = extendType({
  ...
  definition(t) {
    t.crud.booking()
    t.field('findFirstBooking', {
      type: 'Booking',
      args: {
        where: 'BookingWhereInput',
        orderBy: arg({ type: 'BookingOrderByInput' }),
        cursor: 'BookingWhereUniqueInput',
        skip: 'Int',
        take: 'Int',
      },
      async resolve(_root, args, ctx) {
        return ctx.prisma.booking.findFirst(args as any)
      },
    })
    t.crud.bookings({ filtering: true, ordering: true })
    t.field('bookingsCount', {
      type: 'Int',
      args: {
        where: 'BookingWhereInput',
      },
      async resolve(_root, args, ctx) {
        return ctx.prisma.booking.count(args as any)
      },
    })
  },
})

export const bookingMutation = extendType({
  type: 'Mutation',
  definition(t) {
    t.crud.createOneBooking()
    t.crud.updateOneBooking()
    t.crud.upsertOneBooking()
    t.crud.deleteOneBooking()
    t.crud.updateManyBooking()
  },
})
