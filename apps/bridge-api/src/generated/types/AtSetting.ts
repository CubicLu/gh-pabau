...
export const atSettingQuery = extendType({
  ...
  definition(t) {
    t.crud.atSetting()
    t.field('findFirstAtSetting', {
      type: 'AtSetting',
      args: {
        where: 'AtSettingWhereInput',
        orderBy: arg({ type: 'AtSettingOrderByInput' }),
        cursor: 'AtSettingWhereUniqueInput',
        skip: 'Int',
        take: 'Int',
      },
      async resolve(_root, args, ctx) {
        return ctx.prisma.atSetting.findFirst(args as any)
      },
    })
    ...
    t.field('atSettingsCount', {
      ...
      async resolve(_root, args, ctx) {
        return ctx.prisma.atSetting.count(args as any)
      },
    })
  },
})
