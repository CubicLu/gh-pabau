      ...
      async resolve(_root, input, ctx: Context) {
        ...
        (
          ...
          WHERE  a.occupier= ${ctx.authenticated.company} and a.reference_no not in ("**REFUND**", "**CREDIT NOTE**", "ACCOUNT PAYMENT")
            ...
            AND a.refund_to = 0 AND b.product_category_type in ("service", "retail", "packages")
          UNION
          SELECT s.id,
            s.date AS date,
      ...
      async resolve(_root, input, ctx: Context) {
        const invoices = await ctx.prisma
          .$queryRaw`SELECT sum(count) as count from (
            ...
            FROM   inv_sales a
            LEFT JOIN inv_sale_items b ON a.id = b.sale_id
            WHERE  a.occupier= ${ctx.authenticated.company} and a.reference_no not in ("**REFUND**", "**CREDIT NOTE**", "ACCOUNT PAYMENT") AND a.customer_id = ${input?.contact_id} AND a.refund_to = 0 AND b.product_category_type in ("service", "retail", "packages")
              UNION
