...
export interface CompareSliderViewerProps {
  imageSrc: string
  zoomInMode: ZoomInMode
  date?: string
  datePos?: 'left' | 'right' | 'top' | 'bottom'
  dragging?: boolean
  width: number
  height: number
  isLeft?: boolean
...
export const CompareSliderViewer: FC<CompareSliderViewerProps> = ({
  imageSrc = '',
  dragging,
  date = '',
  datePos = 'bottom',
  ...
  }, [zoomInMode, imageSrc, width, height, isLeft])
  ...
  useEffect(() => {
    if (imageSrc === '') {
      ...
      img.addEventListener('load', () => {
        ...
        if (isLeft) {
          ctx?.drawImage(img, 0, 0, sWidth / 2, sHeight, 0, 0, width, height)
        } else {
          ctx?.drawImage(
            img,
            sWidth / 2,
  ...
  }, [imageSrc, width, height, isLeft, imgUrl])
